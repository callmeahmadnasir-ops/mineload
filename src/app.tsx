...
const MINE_BUTTON = 'mine'
const DONE_LABEL = '↵ mine another'
const TAGLINE = 'mine any video. paste. mine. done.'
...
function AppContent({
  ...
  const [phase, setPhase] = useState<Phase>(initialUrl ? {name: 'probing', status: 'crafting pickaxe…'} : {name: 'input'})
  ...
  const startProbe = useCallback(async (targetUrl: string) => {
    ...
    setPhase({name: 'probing', status: 'crafting pickaxe…'})
    try {
      const ytdlp =
        ytdlpRef.current ||
  ...
  if (phase.name === 'input') {
    ...
    clickTargets.push({match: `  ${MINE_BUTTON}  `, padY: 1, action: () => handleUrlSubmit(urlInput)})
  }
    ...
    <FullScreen>
      ...
      {phase.name === 'input' && (
        <Box flexDirection="column" alignItems="center">
          <FramedInput title="Paste a link" width={boxWidth} button={MINE_BUTTON}>
            <TextInput
              value={urlInput}
          ...
          {phase.warning ? (
            ...
            <Text color={theme.gray} dimColor={theme.dimSecondary}>from your clipboard — ↵ to mine it</Text>
          ) : null}
        </Box>
      )}
      ...
      {phase.name === 'probing' && (
        <Box flexDirection="column" alignItems="center">
          <FramedInput title={platform ? platform.label : 'Paste a link'} width={boxWidth} button={MINE_BUTTON} buttonDim>
          ...
          {phase.processing ? (
            ...
            <>
              <Text>
                ...
                <Text color={theme.gray} dimColor={theme.dimSecondary}> mining…</Text>
              </Text>
              <Gap />
            ...
            <>
              ...
              <Text>
                ...
                <Text color={theme.gray} dimColor={theme.dimSecondary}>
                  {phase.refreshing ? ' link expired — grabbing a fresh one…' : ' starting to mine…'}
                </Text>
              </Text>
            </>
          )}
      ...
      {phase.name === 'done' && (
        <Box flexDirection="column" alignItems="center">
          <Text>
            <Text bold color={theme.primary}>✓ mined! </Text>
            <Text color={theme.primary}>find your loot in:</Text>
          </Text>
